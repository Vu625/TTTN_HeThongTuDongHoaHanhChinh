...
def load_embeddings_cached(prefix):
    return np.load(BASE_DIR / f"data/index/{prefix}_embeddings.npy")

@st.cache_resource
def load_chunks_cached(prefix):
    with open(BASE_DIR / f"data/index/{prefix}_chunks.pkl", "rb") as f:
        return pickle.load(f)

@st.cache_resource
def load_filemap_cached(prefix):
    with open(BASE_DIR / f"data/index/{prefix}_filemap.pkl", "rb") as f:
        return pickle.load(f)

@st.cache_resource
def load_tfidf_cached(prefix):
    from scipy.sparse import load_npz
    return load_npz(BASE_DIR / f"data/index/{prefix}_tfidf.npz")

@st.cache_resource
def load_vectorizer_cached(prefix):
    with open(BASE_DIR / f"data/index/{prefix}_vectorizer.pkl", "rb") as f:
        return pickle.load(f)
